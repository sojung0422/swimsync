                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select className={`${inputCls} cursor-pointer`} value={form.level} onChange={e => set('level', e.target.value)}>
                      {settings.swimLevels.map(l => <option key={l}>{l}</option>)}
                    </select>
                  </div>
                  <div>
...
function LevelTestModal({ student, onClose }: { student: Student; onClose: () => void }) {
  const { instructors, settings, recordLevelTest } = useStore();
  const [instructorId, setInstructorId] = useState(student.instructorId || instructors[0]?.id || '');
  const [resultLevel, setResultLevel] = useState(student.level);
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-md shadow-xl">
        <div className="flex items-center justify-between p-6 border-b border-slate-100">
          <h2 className="text-[15px] font-semibold text-slate-800">레벨 테스트 기록 — {student.studentName}</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-700"><X className="w-5 h-5" /></button>
        </div>
        <div className="p-6 space-y-4">
          <div>
          ...
          <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 flex items-center justify-between text-sm">
            <span className="text-slate-500">현재 레벨</span>
            <span className="text-slate-800 font-semibold">{student.level}</span>
          </div>
          <div>
            <label className={labelCls}>결과 레벨</label>
            <select className={`${inputCls} cursor-pointer`} value={resultLevel} onChange={e => setResultLevel(e.target.value)}>
              {settings.swimLevels.map(l => <option key={l}>{l}</option>)}
            </select>
          </div>
          <div>
            <label className={labelCls}>합격 여부</label>
            <div className="flex gap-2">
              <button type="button" onClick={() => setPassed(true)}
                className={`flex-1 py-2.5 rounded-xl text-sm font-medium border transition-colors ${passed ? 'bg-emerald-600 border-emerald-600 text-white' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                합격
              </button>
              <button type="button" onClick={() => setPassed(false)}
                className={`flex-1 py-2.5 rounded-xl text-sm font-medium border transition-colors ${!passed ? 'bg-red-500 border-red-500 text-white' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                불합격
              </button>
            </div>
            {passed && resultLevel !== student.level && (
              <p className="text-emerald-600 text-xs mt-1.5">합격 처리하면 학생의 현재 레벨이 "{resultLevel}"(으)로 바로 반영돼요.</p>
            )}
          </div>
          <div>
            <label className={labelCls}>메모 (선택)</label>
            <textarea className={`${inputCls} resize-none`} rows={2} placeholder="예: 자유형 25m 완주, 배영 보완 필요" value={note} onChange={e => setNote(e.target.value)} />
          </div>
          <div className="flex gap-3">
            <button onClick={onClose} className="flex-1 py-2.5 border border-slate-200 rounded-xl text-slate-600 text-sm hover:bg-slate-50 transition-colors">취소</button>
            <button onClick={() => { recordLevelTest(student.id, instructorId, resultLevel, passed, note.trim()); onClose(); }}
...
function LevelTestSection({ student }: { student: Student }) {
  const { levelTestRecords, instructors } = useStore();
  const [showModal, setShowModal] = useState(false);
  const records = levelTestRecords.filter(r => r.studentId === student.id).slice().reverse();

  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-5">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-slate-700 font-semibold text-sm flex items-center gap-2"><GraduationCap className="w-4 h-4 text-violet-500" /> 레벨 테스트 기록</h3>
        <button onClick={() => setShowModal(true)}
          ...
          <Plus className="w-3.5 h-3.5" /> 테스트 기록 추가
        </button>
      </div>
      {records.length === 0 ? (
        <p className="text-slate-400 text-sm py-2">아직 기록된 레벨 테스트가 없습니다.</p>
      ) : (
        <div className="space-y-2">
          {records.map(r => {
            const inst = instructors.find(i => i.id === r.instructorId);
            return (
              <div key={r.id} className="flex items-center gap-3 bg-slate-50 rounded-xl px-4 py-2.5">
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold border shrink-0 ${r.passed ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-red-50 text-red-600 border-red-200'}`}>
                  {r.passed ? '합격' : '불합격'}
                </span>
                <div className="flex-1 min-w-0">
                  <p className="text-slate-700 text-sm font-medium">{r.previousLevel || '-'} → {r.resultLevel}</p>
                  {r.note && <p className="text-slate-400 text-xs mt-0.5">{r.note}</p>}
                </div>
                <div className="text-right shrink-0">
                  <p className="text-slate-400 text-xs">{r.testDate}</p>
                  <p className="text-slate-300 text-[11px]">{inst?.name ?? '-'}</p>
                </div>
              </div>
            );
          })}
      ...
      {showModal && <LevelTestModal student={student} onClose={() => setShowModal(false)} />}
    </div>
  );
}
              ...
              <div className="bg-violet-50 border border-violet-100 rounded-xl p-3 mb-2 space-y-1.5">
                ...
                <div className="flex items-center justify-between pt-1">
                  ...
                  <button onClick={() => updateStudent(student.id, { paymentAmount: suggestedAmount })}
                    className="px-2.5 py-1 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-[11px] font-medium transition-colors">
                    결제 금액에 적용
                  </button>
                </div>
              </div>
...
function AddWaitlistModal({ onClose }: { onClose: () => void }) {
  const { lessonClasses, addWaitlistEntry } = useStore();
  const [studentName, setStudentName] = useState('');
  const [parentPhone, setParentPhone] = useState('');
  const [lessonClassId, setLessonClassId] = useState(lessonClasses[0]?.id ?? '');
  ...
  const inputCls = "w-full border border-slate-200 rounded-xl px-3 py-2.5 text-slate-800 placeholder:text-slate-400 text-sm focus:outline-none focus:border-cyan-500 transition-colors";
  const labelCls = "block text-xs font-medium text-slate-500 mb-1";
  const toggleDay = (d: string) => setDesiredDays(prev => prev.includes(d) ? prev.filter(x => x !== d) : [...prev, d]);

  const handleSave = () => {
    if (!studentName.trim() || !lessonClassId) return;
    addWaitlistEntry({ studentName: studentName.trim(), parentPhone: parentPhone.trim(), lessonClassId, desiredDays, desiredTime, category, note: note.trim() });
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-md shadow-xl">
        <div className="flex items-center justify-between p-6 border-b border-slate-100">
          <h2 className="text-[15px] font-semibold text-slate-800">대기자 등록</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-700"><X className="w-5 h-5" /></button>
        </div>
        <div className="p-6 space-y-4">
          <div>
            <label className={labelCls}>이름 *</label>
            <input className={inputCls} placeholder="문의자(원생) 이름" value={studentName} onChange={e => setStudentName(e.target.value)} />
          </div>
          <div>
            <label className={labelCls}>연락처</label>
            <input className={inputCls} placeholder="010-0000-0000" value={parentPhone} onChange={e => setParentPhone(e.target.value)} />
          </div>
          <div>
            <label className={labelCls}>희망 강습반 *</label>
            ...
            <div className="flex gap-1.5">
              {['월', '화', '수', '목', '금', '토', '일'].map(d => (
                <button key={d} type="button" onClick={() => toggleDay(d)}
                  className={`flex-1 py-2 rounded-lg text-xs font-medium border transition-colors ${desiredDays.includes(d) ? 'bg-cyan-600 border-cyan-600 text-white' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'}`}>
                  {d}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <input type="time" className={inputCls} value={desiredTime} onChange={e => setDesiredTime(e.target.value)} />
          </div>
          <div>
            <label className={labelCls}>메모 (선택)</label>
            <input className={inputCls} placeholder="예: 체험 문의, 형제 있음 등" value={note} onChange={e => setNote(e.target.value)} />
          </div>
          <div className="flex gap-3">
            <button onClick={onClose} className="flex-1 py-2.5 border border-slate-200 rounded-xl text-slate-600 text-sm hover:bg-slate-50 transition-colors">취소</button>
            <button onClick={handleSave} disabled={!studentName.trim() || !lessonClassId}
              className="flex-1 py-2.5 bg-cyan-600 hover:bg-cyan-700 disabled:opacity-40 text-white rounded-xl text-sm font-medium transition-colors">
              등록
            </button>
          </div>
        </div>
...
function WaitlistView() {
  ...
  const remainingSeats = (lessonClassId: string) => {
    ...
    if (!lc) return 0;
    const enrolled = students.flatMap(getAllEnrollments).filter(e => e.status === 'active' && e.lessonClassId === lessonClassId).length;
    return lc.capacity - enrolled;
  };

  const filtered = waitlistEntries.filter(w => filter === 'all' || w.status === 'waiting').slice().reverse();
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          {(['waiting', 'all'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-1.5 rounded-xl text-xs font-medium border transition-colors ${filter === f ? 'bg-cyan-50 border-cyan-200 text-cyan-700' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'}`}>
              {f === 'waiting' ? '대기 중' : '전체'}
            </button>
          ))}
        </div>
        <button onClick={() => setShowAdd(true)}
          ...
          <UserPlus className="w-3.5 h-3.5" /> 대기자 등록
        </button>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-2xl py-16 text-center text-slate-400">
          ...
          <p className="text-sm">{filter === 'waiting' ? '대기 중인 문의가 없습니다.' : '등록된 대기자가 없습니다.'}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3 items-start">
          {filtered.map(w => {
            const lc = lessonClasses.find(l => l.id === w.lessonClassId);
            const seats = remainingSeats(w.lessonClassId);
            const seatOpen = seats > 0;
            return (
              <div key={w.id} className="bg-white border border-slate-200 rounded-2xl p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <p className="text-slate-800 text-sm font-semibold">{w.studentName}</p>
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold border bg-slate-50 text-slate-500 border-slate-200">{w.category === 'child' ? '아동' : '성인'}</span>
                    </div>
                    <p className="text-slate-400 text-xs mt-0.5">{lc?.name ?? '-'} · {w.desiredDays.join('·') || '요일 미정'} {w.desiredTime}</p>
                  </div>
                  {w.status !== 'waiting' && (
                    <button onClick={() => deleteWaitlistEntry(w.id)} className="text-slate-300 hover:text-red-500 transition-colors p-1 shrink-0">
                ...
                <div className="flex items-center gap-1.5 flex-wrap mt-2">
                  {w.status !== 'waiting' && (
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold border ${w.status === 'converted' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : w.status === 'notified' ? 'bg-cyan-50 text-cyan-700 border-cyan-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                      {w.status === 'converted' ? '등록 완료' : w.status === 'notified' ? '안내함' : '취소됨'}
                    </span>
                  )}
                  {seatOpen && w.status === 'waiting' && (
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold border bg-red-50 text-red-600 border-red-200 animate-pulse">자리 났어요 · 잔여 {seats}자리</span>
                  )}
                </div>
                {w.parentPhone && <p className="text-slate-400 text-xs mt-1.5 flex items-center gap-1"><Phone className="w-3 h-3" /> {w.parentPhone}</p>}
                {w.note && <p className="text-slate-400 text-xs mt-1.5 bg-slate-50 rounded-lg px-2.5 py-1.5">{w.note}</p>}
                <p className="text-slate-300 text-[11px] mt-1.5">등록: {w.requestedAt}</p>
                {w.status === 'waiting' && (
                  <div className="flex items-center gap-1.5 mt-3">
                    <button onClick={() => markWaitlistNotified(w.id)}
                      className="flex-1 flex items-center justify-center gap-1.5 px-2.5 py-1.5 bg-cyan-50 hover:bg-cyan-100 border border-cyan-200 text-cyan-700 rounded-lg text-xs font-medium transition-colors">
                      <Bell className="w-3.5 h-3.5" /> 안내함
                    </button>
                    <button onClick={() => setRegisterTarget(w)}
                      className="flex-1 flex items-center justify-center gap-1.5 px-2.5 py-1.5 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-emerald-700 rounded-lg text-xs font-medium transition-colors">
                      <UserPlus className="w-3.5 h-3.5" /> 등록
                    </button>
                    <button onClick={() => cancelWaitlistEntry(w.id)}
                      className="shrink-0 flex items-center justify-center px-2.5 py-1.5 bg-red-50 hover:bg-red-100 border border-red-200 text-red-600 rounded-lg text-xs font-medium transition-colors">
...
export default function AdminStudents() {
  const { students, addStudent, updateStudent, deleteStudent, extendStudentClasses, deferStudentClasses, waitlistEntries } = useStore();

  const [view, setView] = useState<'by-class' | 'all' | 'waitlist'>('by-class');
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      ...
      {!detailStudent && !searchResults && (
        <div className="shrink-0 flex border-b border-slate-200 bg-white px-6">
          ...
          <button onClick={() => setView('waitlist')}
            className={`flex items-center gap-1.5 px-4 py-3 text-sm font-medium border-b-2 transition-colors relative ${view === 'waitlist' ? 'border-cyan-600 text-cyan-700' : 'border-transparent text-slate-400 hover:text-slate-700'}`}>
            <Hourglass className="w-4 h-4" /> 대기자 명단
            {waitingCount > 0 && (
              <span className="ml-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-red-500 text-white">{waitingCount}</span>
            )}
          </button>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!searchResults && !detailStudent && view === 'waitlist' && <WaitlistView />}
      </div>
