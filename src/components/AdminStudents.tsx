      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2.5">
          {filtered.map(w => {
            ...
            return (
              <div key={w.id} className="bg-white border border-slate-200 rounded-2xl p-4 flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-slate-400 text-xs mt-1">{lc?.name ?? '-'} · {w.desiredDays.join('·') || '요일 미정'} {w.desiredTime}</p>
                  {w.parentPhone && <p className="text-slate-400 text-xs mt-0.5 flex items-center gap-1"><Phone className="w-3 h-3" /> {w.parentPhone}</p>}
                  {w.note && <p className="text-slate-400 text-xs mt-1 bg-slate-50 rounded-lg px-2.5 py-1.5 inline-block">{w.note}</p>}
