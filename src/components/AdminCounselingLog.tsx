...
import { NotebookPen, Plus, Trash2, X, Send, Settings2, ArrowRightCircle } from 'lucide-react';
...
function BulkSmsModal({ recipients, onClose }: { recipients: LeadRecord[]; onClose: () => void }) {
  const { addNotification, sendNotification } = useStore();
  return (
    <BulkMessageModal
      ...
      onSend={(content, images) => {
        ...
        const id = addNotification({ type: 'event', title: '이벤트 안내 (상담일지 대상 발송)', content, recipientIds: [], recipientPhones: phones, images });
        sendNotification(id);
      }}
    />
  );
